...
### Étapes à suivre pour faire fonctionner `prettier/prettier` avec Flat Config

#### 1. Installe les bonnes dépendances

```bash
npm install -D prettier eslint-plugin-prettier eslint-config-prettier
```

#### 2. Modifie ton fichier `eslint.config.js` pour activer Prettier **en tant que plugin ESLint**

Voici une version **corrigée et complète** :
...
#### 3. Ajoute ou vérifie `.prettierrc.json` :
...
#### 4. Test avec ce fichier `src/test-lint.ts` :
